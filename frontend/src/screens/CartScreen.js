...
import {
  addCartItems,
  removeCartItems,
} from "../redux-utils/actions/cartActions";
...
import Message from "../utils/AlertMessage";
 
export const CartScreen = ({ match, location, history }) => {
   const dispatch = useDispatch();
  
  const productId = match.params._id
  const quantity = location.search ? Number(location.search.split('=')[2]) : 1
  ...
  useEffect(() => {
    if (productId) {
      dispatch(addCartItems(productId, quantity))
    }
